...
if "%uploadTo_NEW_DEMO_AWS%" == "y" (
	echo ' '
	echo ---------- start upload all files AAAAA ----------
	call %SCP_Home% -i %SCP_Identity_File% %SourceCodePath%\CommonBase\target\CommonBase-1.0-SNAPSHOT.jar %NewDemoAwsAuth%:/pkmj/upload/
	call %SCP_Home% -i %SCP_Identity_File% %SourceCodePath%\ServerCore\target\ServerCore-1.0-SNAPSHOT.jar %NewDemoAwsAuth%:/pkmj/upload/
	call %SCP_Home% -i %SCP_Identity_File% %SourceCodePath%\LobbyServer\target\LobbyServer-1.0-SNAPSHOT.jar %NewDemoAwsAuth%:/pkmj/upload/
	call %SCP_Home% -i %SCP_Identity_File% %SourceCodePath%\GameServer\target\GameServer-1.0-SNAPSHOT.jar %NewDemoAwsAuth%:/pkmj/upload/
	call %SCP_Home% -i %SCP_Identity_File% %SourceCodePath%\HttpServer\target\HttpServer.war %NewDemoAwsAuth%:/pkmj/upload/
	call %SCP_Home% -i %SCP_Identity_File% %SourceCodePath%\Backoffice\target\ROOT.war %NewDemoAwsAuth%:/pkmj/upload/
	call %SCP_Home% -i %SCP_Identity_File% %SourceCodePath%\AiRobot\target\AiRobot-1.0-SNAPSHOT.jar %NewDemoAwsAuth%:/pkmj/upload/
	call %SCP_Home% -i %SCP_Identity_File% %SourceCodePath%\DataConsolidate\target\DataConsolidate-1.0-SNAPSHOT.jar %NewDemoAwsAuth%:/pkmj/upload/
	echo ---------- finish upload all files BBBBBB ----------
	echo ' '
)
